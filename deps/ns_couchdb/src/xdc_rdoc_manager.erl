...
handle_call({link, replicator, Pid}, _From, State) ->
    undefined = State#state.ddoc_replicator,
    erlang:link(Pid),
    {reply, self(), State#state{ddoc_replicator = Pid}};
handle_call({link, rep_manager, Pid}, _From, State) ->
    undefined = State#state.rep_manager,
    erlang:link(Pid),
    {reply, self(), State#state{rep_manager = Pid}};
...
-spec link(replicator | replication_srv | rep_manager, pid()) -> pid().
link(Type, Pid) ->
