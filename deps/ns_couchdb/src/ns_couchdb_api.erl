...
fetch_stats(BucketName) ->
    maybe_rpc_couchdb_node({fetch_stats, BucketName}).
...
-spec link_to_doc_mgr(atom(), ext_bucket_name() | xdcr, pid()) -> pid().
...
handle_rpc({fetch_stats, BucketName}) ->
...
handle_rpc({link_to_doc_mgr, Type, xdcr, Pid}) ->
    xdc_rdoc_manager:link(Type, Pid);
handle_rpc({link_to_doc_mgr, Type, Bucket, Pid}) ->
    capi_set_view_manager:link(Type, Bucket, Pid).
