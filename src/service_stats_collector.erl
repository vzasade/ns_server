...
process_stats(TS, GrabbedStats, PrevCounters, PrevTS, #state{service = Service,
    ...
    AggregatedStats =
        [{Prefix ++ binary_to_list(Bucket), Values} ||
            {Bucket, Values} <- aggregate_index_stats(Service, Stats, KnownBuckets, Defaults)] ++
        ServiceStats,
...
do_aggregate_index_stats(Service, [{{Bucket, _, _}, _} | _] = Stats,
    ...
    {BucketStats, RestStats} = aggregate_index_bucket_stats(Service, Bucket, Stats, Defaults),
...
do_aggregate_index_bucket_stats(Service, Acc, Bucket, [{{Bucket, Index, Name}, V} | Rest]) ->
