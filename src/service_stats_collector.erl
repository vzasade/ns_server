...
%% API
-export([start_link/1, service_prefix/1, service_event_name/1, global_stat/2, per_item_stat/3]).
...
start_link(Service) ->
    base_stats_collector:start_link({local, server_name(Service)}, ?MODULE, Service).
...
init(Service) ->
    ...
    Defaults = [{global_stat(Service, atom_to_binary(Stat, latin1)), 0}
                || Stat <- Service:get_gauges() ++ Service:get_counters() ++ Service:get_computed()],
...
do_recognize_name(Service, K) ->
    Type = find_type(K, [{#stats_accumulators.sys_gauges, Service:get_service_gauges()},
                         {#stats_accumulators.sys_counters, Service:get_service_counters()}]),
    case Type of
        not_found ->
            do_recognize_bucket_metric(Service, K);
        _ ->
            NewKey = list_to_binary(service_stat_prefix(Service) ++ binary_to_list(K)),
            {Type, NewKey}
    end.
...
massage_stats(Service, Ets, [{K, V} | Rest], Acc) ->
    case recognize_name(Service, Ets, K) of
        ...
        {Pos, NewK} ->
            massage_stats(
              Service, Ets, Rest, setelement(Pos, Acc, [{NewK, V} | element(Pos, Acc)]))
    end.

grab_stats(#state{service = Service}) ->
    case ns_cluster_membership:should_run_service(ns_config:latest(), Service:get_type(), node()) of
        true ->
...
do_grab_stats(Service) ->
    case Service:grab_stats() of
        {ok, {[_|_] = Stats}} ->
            Stats;
        {ok, Other} ->
            ?log_error("Got invalid stats response for ~p:~n~p", [Service, Other]),
            [];
        {error, _} ->
            []
    end.

process_stats(TS, GrabbedStats, PrevCounters, PrevTS, #state{service = Service,
    ...
    service_status_keeper:update(Service, MassagedStats#stats_accumulators.status),
    ...
    ServiceStats = [{service_event_name(Service), finalize_stats(ServiceStats1)}],
    Prefix = service_prefix(Service),
    AggregatedStats =
        [{Prefix ++ binary_to_list(Bucket), Values} ||
            {Bucket, Values} <- aggregate_stats(Service, Stats, KnownBuckets, Defaults)] ++
        ServiceStats,
...
do_aggregate_stats(Service, [{{Bucket, _, _}, _} | _] = Stats,
    ...
    {BucketStats, RestStats} = aggregate_bucket_stats(Service, Bucket, Stats, Defaults),
...
do_aggregate_bucket_stats(_Service, Acc, _, []) ->
    {finalize_stats(Acc), []};
do_aggregate_bucket_stats(Service, Acc, Bucket, [{{Bucket, Item, Name}, V} | Rest]) ->
