...
-ifdef(DEBUG_UPR).
-define(print_proxied_packet(X), ?rebalance_info("Proxy packet: ~p", [format_packet_nicely(Packet)])).
-else.
...
format_packet_nicely(<<?REQ_MAGIC:8, _Rest/binary>> = Packet) ->
    ...
    format_packet_nicely("REQUEST", "", Header, Body);
...
format_packet_nicely(Type, Status, Header, Body) ->
    lists:flatten(
      io_lib:format("~s: ~.16X (~w) vbucket = ~w opaque = ~.16X~s BODY: ~w",
                    [Type,
