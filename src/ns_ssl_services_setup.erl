...
get_user_set_chain_and_pkey() ->
    case [file:read_file(user_set_cert_path()),
          file:read_file(user_set_key_path()),
          file:read_file(user_set_ca_chain_path())] of
        [{ok, Cert}, {ok, PKey}, {ok, CAChain}] ->
            {Cert, PKey, CAChain};
        Other ->
            Sanitized =
                lists:map(fun ({ok, _}) ->
            ...
            ?log_debug("User set ca, cert or pkey not found. Use generated certificate. Errors: ~p",
                       [Sanitized]),
            use_generated
    end.
