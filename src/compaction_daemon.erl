...
init([]) ->
    ...
    named_queues:start_link(compaction_queues),
...
spawn_vbucket_compactor(BucketName, {VBucket, _DbName} = VBucketAndDb,
                        Config, Force, Options) ->
    proc_lib:spawn_link(
      fun () ->
              case named_queues:execute_task(compaction_queues,
                                             {BucketName, VBucket},
                                             fun maybe_compact_vbucket/1,
                                             [BucketName, VBucketAndDb, Config, Force, Options]) of
                  ok ->
                      ok;
                  Result ->
                      exit(Result)
...
aggregated_size_info(Bucket) ->
    {ok, {DS, FS}} =
        ns_memcached:raw_stats(node(), Bucket, <<>>,
