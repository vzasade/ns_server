...
init({Src, Dst, Opts}=InitArgs) ->
    ...
    {UpstreamAux, DownstreamAux} =
        case OldState of
            undefined ->
                {mc_socket:connect(Src, Username, Password, Bucket),
                 mc_socket:connect(Dst, Username, Password, Bucket)};
            _ ->
    ...
    {Upstream, Args} =
        case OldState =:= undefined orelse
            ...
            true ->
                Upstream0 = mc_socket:connect(Src, Username, Password, Bucket),
...
process_data(Data, Elem, CB, State) ->
    mc_socket:process_data(Data, Elem,
