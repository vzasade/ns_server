...
get_stats_search_order(StatName, BucketName) ->
    lists:foldl(
      fun (Section, AllSections) ->
              case lists:delete(Section, AllSections) of
                  AllSections ->
                      AllSections;
                  Other ->
                      [Section | Other]
      ...
      ["@system", BucketName, "@xdcr-" ++ BucketName] ++ services_sections(BucketName),
      guess_sections_by_pefix(StatName, BucketName)).
...
section_nodes(Section) ->
    case describe_section(Section) of
        {Service, _} ->
            ns_cluster_membership:service_actual_nodes(ns_config:latest(), Service);
        undefined ->
            %% Section is a bucket
...
couchbase_index_stats_descriptions(BucketId, ServiceNodes) ->
    proceed_if_has_nodes(index, ServiceNodes, {stats_directory_index, BucketId},
                         fun (IndexNodes) ->
...
couchbase_fts_stats_descriptions(BucketId, ServiceNodes) ->
    proceed_if_has_nodes(fts, ServiceNodes, {stats_directory_fts, BucketId},
                         fun (FtsNodes) ->
    ...
    [{blockName, <<"Server Resources">>},
     ...
     {stats,
      [{struct, [{isBytes, true},
       ...
       | index_server_resources_stats_description(has_nodes(index, ServiceNodes)) ++
           fts_server_resources_stats_description(has_nodes(fts, ServiceNodes)) ++
           cbas_server_resources_stats_description(has_nodes(cbas, ServiceNodes))]}].
...
serve_aggregated_ui_stats(Req, Params) ->
    ...
    ServiceNodes =
        lists:map(
          fun ({Service, Param}) ->
                  AllNodes = ns_cluster_membership:service_actual_nodes(ns_config:latest(), Service),
                  {Service,
                   {Param,
                  ...
                  case proplists:get_value(Service, ServiceNodes) of
                      ...
                      {_, N1} ->
                          [{list_to_binary(Section),
                            {grab_ui_stats(Section, N1, HaveStamp, Wnd)}} | AccStats]
                  end
    ...
    StatsDirectoryV =
        erlang:phash2(
          base_stats_directory(Bucket, [{Service, N} || {Service, {_, N}} <- ServiceNodes])),
