    ...
    work_queue:submit_work(
      ...
      fun () ->
              ...
              case Service:restart() of
                  ...
                  Error ->
                      ?log_error("Failed to restart the ~p: ~p", [Service, Error])
              end,
    ...
    case Nodes of
        ...
        _ ->
            ...
            try Service:get_remote_items(Node) of
                ...
                Error ->
                    ?log_error("Couldn't get items from node ~p: ~p", [Node, Error]),
                    {error, failed}
            catch
                T:E ->
                    ?log_error("Got exception while getting items from node ~p: ~p",
                               [Node, {T, E}]),
                    {error, failed}
            end
    end.
...
get_source(Service) ->
    ...
    case ns_cluster_membership:should_run_service(Config, Service:get_type(),
        ...
        true ->
            local;
        false ->
            ServiceNodes = ns_cluster_membership:service_actual_nodes(Config, Service:get_type()),
...
notify_change(#state{service = Service,
                     version = Version} = State) ->
    gen_event:notify(index_events, {indexes_change, Service:get_type(), Version}),
    State.
